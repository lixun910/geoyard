...
complement.cumdist <- function(xs) {
  CCD <- list()
  ...
  pdfv <- array(0,dim=n)
  for (i in c(1:n)) {
    pdfv[i] <- length(which(xs==pdfk[i])) / length(xs)
  }  
  cdfv <- cumsum(pdfv)
  for (i in c(1:n)) {
    CCD[as.character(pdfk[i])] <- max(cdfv) - cdfv[i]
  }
  return(CCD)
}
...
# Using KS-test to choose best fit for pareto distribution
pareto.bestfit <- function(data) {
  if (length(data) > 500){
    #resampling 
  ...
  S <- complement.cumdist(data)
  n <- length(xmins)
  alphas <- array(0,dim=n)
  likelihoods<- array(0,dim=n)
  Ds <- array(0,dim=n)

  for (i in c(1:n)) {
    xmin <- xmins[i]
    data_xmin <- data[data >= xmin] 
    fit<- pareto.fit(data_xmin, xmin)
    alpha<-fit$exponent
    likelihood<- fit$loglike
    ...
    for (j in c(1:m)) {
      x <- xs[j]
      P[j] <- ppareto(x,xminhat,alpha,FALSE,FALSE)
    }
    alphas[i] <-alpha
  ...
  xmin <- xmins[idx]
  alpha <- alphas[idx]
  loglikeli <- likelihoods[idx]

  return(list(exponent=alpha,minx=xmin,loglike=loglikeli,alpha_list=alphas,ks_list=Ds))
}
