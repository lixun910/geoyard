...
# integral function
exp_int_function_filename <- "~/powerlawR/exponential_integral"
#### EDIT THIS LOCATION!!!! ###
...
# for geophoto test
##########################
print_TPL <- function(TPL.data,count,sample) {
  TPL.data <- paste(TPL.data, count, sep="")
  sample.data <- paste(unlist(sample),collapse=",")
  TPL.data <- paste(TPL.data, sample.data, sep=",")
  TPL.data <- paste(TPL.data,"\n",sep="")
  return (TPL.data)
}  

batchfit <- function(input) {
  results <- c("id,model,n,xmin,exponent,rate,loglikelihood,AIC,wAIC") 
  TPL.data <- ""

  i=1
  dataset <- readLines(input)
  for (data in dataset) {
    data<-strsplit(data,",")
    for(items in data) {items<-as.double(items)}
    n <- length(items)
    # power-law test
    powerlaw <- pareto.bestfit(items)  
    x_min <- powerlaw$minx
    minx <- min(items)
    maxx <- max(items)
    #x_min <- minx

    sample.x <- array(0,198)
    for (j in 1:198) {
      sample.x[j]  <- as.integer(minx) + j*(as.integer(maxx) - as.integer(minx))/200 
    }
    sample.x  <- c(minx,sample.x,maxx)
    m <- length(sample.x)

    # origin data
    TPL.data <- print_TPL(TPL.data,i,items) 
    # curve sample x
    TPL.data <- print_TPL(TPL.data,i,sample.x) 

    powerlaw <- pareto.fit(items,x_min)
    powerlaw.AIC <- 2 - 2*powerlaw$loglike
    results <- paste(results, i, sep="\n")
    results <- paste(results, "power-law",sep=",")
    results <- paste(results, n, sep=",")
    results <- paste(results, x_min, sep=",")
    results <- paste(results, powerlaw$exponent, sep=",")
    results <- paste(results, "N/A", sep=",")
    results <- paste(results, powerlaw$loglike, sep=",")
    results <- paste(results, powerlaw.AIC, sep=",")

    # power-law curve
    sample.p <- ppareto(sample.x,minx,powerlaw$exponent,FALSE,FALSE)
    TPL.data <- print_TPL(TPL.data,i,sample.p) 

    # exponential test
    exponential <- exp.fit.moment(items,x_min)
    exp.AIC <- 2 - 2*exponential$loglike
    results <- paste(results, i, sep="\n")
    results <- paste(results, "exponential",sep=",")
    results <- paste(results, n, sep=",")
    results <- paste(results, x_min, sep=",")
    results <- paste(results, "N/A", sep=",")
    results <- paste(results, exponential$rate, sep=",")
    results <- paste(results, exponential$loglike, sep=",")
    results <- paste(results, exp.AIC, sep=",")

    # exp curve
    sample.p <- pexp(sample.x, exponential$rate,FALSE,FALSE)
    TPL.data <- print_TPL(TPL.data,i,sample.p) 

    # truncated power-law
    #powerexp<-powerexp.fit(items,x_min)
    powerexp<-powerexp.bestfit(items)
    x_min <- powerexp$minx
    powerexp.AIC <- 2 - 2*powerexp$loglike
    results <- paste(results, i, sep="\n")
    results <- paste(results, "truncated power-law",sep=",")
    results <- paste(results, n, sep=",")
    results <- paste(results, x_min, sep=",")
    results <- paste(results, powerexp$exponent, sep=",")
    results <- paste(results, powerexp$rate, sep=",")
    results <- paste(results, powerexp$loglike, sep=",")
    results <- paste(results, powerexp.AIC, sep=",")
  
    # origin data.p
    items.p  <- ppowerexp(items,x_min,powerexp$exponent, powerexp$rate,FALSE,FALSE) 
    TPL.data <- print_TPL(TPL.data,i,items.p) 
    # truncated power-law curve
    sample.p <- ppowerexp(sample.x,minx,powerexp$exponent,powerexp$rate,FALSE,FALSE)
    TPL.data <- print_TPL(TPL.data,i,sample.p) 
    # others
    TPL.data <- paste(TPL.data, i, sep="")
    TPL.data <- paste(TPL.data, n, x_min,powerlaw$exponent,powerlaw.AIC,exponential$rate,exp.AIC, powerexp$exponent,powerexp$rate, powerexp.AIC,sep=",")   
    TPL.data <- paste(TPL.data,"\n",sep="")

    i <- i+1
  }  
  write.table(results, "results.csv",row.names=FALSE,col.names=FALSE,quote=FALSE)
  write.table(TPL.data, "TPLdata.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)
}

powerexp.bestfit <- function(data) {
  if (length(data) > 500){
    #resampling 
    data <- sample(data, 500)
  }
  xmins <- sort(unique(data))
  xmins <- xmins[1:length(xmins)-1]
  xminhat <- min(xmins)

  n <- length(xmins)
  exponents<- array(0,dim=n)
  rates <- array(0,dim=n)
  Ds <- array(Inf,dim=n)
  Ls <- array(0,dim=n)
  ...
  for (i in range) {
    ...
    fit <- powerexp.fit(data_xmin, xmin)
    alpha <- fit$exponent
    lamda <- fit$rate  
    loglike <- fit$loglike
    if (alpha==0 || lamda==0) {continue}
    Sxmin <- complement.cumdist(data_xmin)
    xs <- as.numeric(names(Sxmin))
    m <- length(xs)
    P <- array(0, dim=m)
    for (j in c(1:m)) {
      x <- xs[j]
      P[j] <- ppowerexp(x, xminhat,alpha,lamda,FALSE,FALSE)
    }
    exponents[i] <-alpha
    rates[i] <- lamda
    Ds[i]<- KSStat(as.numeric(Sxmin), P)
    Ls[i]<- loglike
  }

  ksmin <- min(Ds)
  idx <- which(Ds==ksmin)[1]
  idx <- 1
  xmin <- xmins[idx]
  alpha <- exponents[idx]
  lamda <- rates[idx]
  ...
  xstart<- xmin
  xend <- max(data)*1.2
  samples <-(xend-xstart)/c(1000:1) +xstart
  p_samples <- array(0,dim=1000)
  for (i in c(1:1000)){
    x <- samples[i]
    p_samples[i] <- ppowerexp(x,xmin,alpha,lamda,FALSE,FALSE)
  }
  return(list(x=sort(data),y=c(length(data):1)/length(data),samplex=samples,sampley=p_samples,exponent=alpha,rate=lamda, minx=xmin,loglike=loglike, AIC=AIC))#, xmin_list=xmins,exponent_list=exponents,rate_list=rates,ks_list=Ds,loglike_list=Ls))
}

test.read_record <- function(path){
  conn <- file(path)
  content <- readLines(con)
  for (line in content){
    line <- unlist(strsplit(line,','))
    data <- as.numeric(line)  
    # power-law test
    powerlaw <- pareto.bestfit(data)  
    x_min <- powerlaw$xmin
    # exponential test
    exponential <- exp.fit(data,x_min)
    # truncated power-law test
        
  }
}

powerexp.plot<-function(data){
  fit <- powerexp.bestfit(data)
  xstart<- fit$minx 
  xend <- max(data)*1.2
  samples <-(xend-xstart)/c(1000:1) +xstart
  p_samples <- array(0,dim=1000)
  for (i in c(1:1000)){
    x <- samples[i]
    p_samples[i] <- ppowerexp(x,fit$minx,fit$exponent,fit$rate,FALSE,FALSE)
  }

  
  plotx <- sort(data)
  ploty <- c(length(data):1) / length(data)

  return(list(x=plotx,y=ploty,samplex=samples,sampley=p_samples))
  #plot(log(plotx),log(ploty))
  #lines(log(samples),log(p_samples),type='l')
}
...
powerexp.fit <- function(data,threshold=1,method="constrOptim",initial_rate=-1) {
  ...
  if (initial_rate < 0) { initial_rate <- exp.fit(data,threshold,"shift")$rate }
  minute_rate <- 1e-6
